	...
	int modificateEmployee (Employee* arrayClienteRecibido, int limiteArrayClienteRecibido)
	  ...
	  int sortEmployees (Employee* arrayClienteRecibido, int limiteArrayClienteRecibido, int ordenObtenida)
	  {
		...
		if (arrayClienteRecibido != NULL && limiteArrayClienteRecibido > 0)
		{
		  ...
		  //printf ("EMPLEADOS EXISTENTES: %d",  cantidadEmpleadosExistentes);
		  ...
		  if (cantidadEmpleadosExistentes > 0)
		  {
			  if (ordenObtenida == 1) //Si la orden de sort es 1/A
			  {
				  ...
				  for (int i = 1 ; i < limiteArrayClienteRecibido ; i++)
				  {
					  //Busco hasta encontrar el cliente con el sector mas bajo
					  if (arrayClienteRecibido[i].sector == i && arrayClienteRecibido[i].isEmpty == 0)
					  {
						  printf ("\n Clientes Sector: %d \n", i);
						  printEmployeesByLastName (arrayClienteRecibido, limiteArrayClienteRecibido, i);
					  }
				  }
			  }
			  ...
			  else if (ordenObtenida == 3) //Si la orden de sort es 3/C
			  {
				// Salir
